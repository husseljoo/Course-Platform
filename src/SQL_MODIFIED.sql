create DATABASE GUCera_DB_Final
...
@cid INT,
@sid INT,
@assignGrade decimal(5,2) OUTPUT
AS
...
as
begin
if(exists(select * from UserMobileNumber where @id=id and @number=mobileNumber))
set @output = 1
else
set @output = 0
end
...
go
create proc courseVerify
@id int,
@cid int,
@output int output
as
begin
if(exists(select * from Course where @cid = id)and exists(select * from StudentTakeCourse where @id = sid and @cid=cid ))
set @output = 1
else
set @output = 0;
end
...
go
create proc assignVerify
@number int,
@type varchar(10),
@output int output
as
begin
if(exists(select * from Assignment where @number = number and @type = type))
set @output = 1
else
set @output = 0;
end
...
as
begin
if(exists(select * from StudentTakeAssignment where @type = assignmenttype and @sid = sid and @cid = cid and @number = assignmentNumber and grade is not null))
set @output = 1
else
set @output = 0
end
...
go
create proc feedbackVerify
@id int,
@cid int,
@output int output
as
begin
if(dbo.checkStudentEnrolledInCourse(@id,@cid)='1' and not exists(select * from Feedback where @id=sid and @cid = cid))
set @output = 1
else
set @output = 0
end
...
--Procedures to be added for Hussein

go
Create proc availableCoursesModified
@sid int
AS
SELECT Course.name, Course.id , StudentTakeCourse.sid  
INTO MyTempTable 
FROM Course 
LEFT OUTER JOIN StudentTakeCourse  ON Course.id = StudentTakeCourse.cid 
WHERE StudentTakeCourse.sid=@sid AND Course.accepted='1'

SELECT Course.name, Course.id , StudentTakeCourse.sid  
INTO MainTable 
FROM Course 
LEFT OUTER JOIN StudentTakeCourse  ON Course.id = StudentTakeCourse.cid 
WHERE Course.accepted='1' AND (StudentTakeCourse.sid IS NULL OR not StudentTakeCourse.sid=@sid)

DELETE MainTable
FROM MainTable
INNER JOIN MyTempTable ON MainTable.id = MyTempTable.id 

Select distinct id as 'CID',name as 'Course Name' FROM MainTable
...
go
create proc checkCreditCardDuplicate
@sid varchar(6),
@number varchar(15),
@output bit output
as
begin
if(exists(select * from StudentAddCreditCard where  @sid=sid and @number=creditCardNumber))
set @output = 1
else
set @output = 0
end
...
go
CREATE PROC viewMyProfileModified
@id int 
AS
BEGIN
IF (EXISTS(Select * FROM Users WHERE id = @id  ))
SELECT * FROM Student INNER JOIN Users ON Student.id = Users.id WHERE @id=Student.id
ELSE
print 'User not found'
END

go
CREATE PROC courseInstId
@cid int,
@instId int output
AS
SELECT @instId=Course.instructorId
FROM Course
where @cid = Course.id
